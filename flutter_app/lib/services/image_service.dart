...
// Platform-specific implementations for save/share/pick
...
class PickedImage {
  ...
  final String? path; // Only available on mobile/desktop
  final String name;
...
class ImageService {
  ...
  /// On desktop, uses native file dialog via file_picker
  /// On mobile, uses image_picker
  /// On web, uses image_picker (browser file dialog)
  static Future<PickedImage?> pickFromGallery() async {
    // On non-web platforms, delegate to platform file which handles desktop vs mobile
    if (!kIsWeb) {
      final result = await platform.pickImageNative();
      if (result != null) return result;
      // Fall through to image_picker if platform picker fails
    }
  ...
  /// Pick an image from camera (not available on web or desktop)
  static Future<PickedImage?> pickFromCamera() async {
    if (kIsWeb || platform.isDesktopPlatform) {
      debugPrint('Camera not available on web/desktop');
      return null;
    }
  ...
  static Uint8List? _compressImageCompute(Map<String, dynamic> params) {
    try {
      ...
      while (quality >= 10) {
        ...
        if (result.length <= targetSizeBytes) return result;
        quality -= 10;
      }
  ...
  /// Save image to gallery (mobile), download (web), or file dialog (desktop)
  ...
  static bool get isCameraAvailable => !kIsWeb && !platform.isDesktopPlatform;
}
